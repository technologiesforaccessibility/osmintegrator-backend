...
public class ExternalServicesConfiguration : IExternalServicesConfiguration
{
  ...
  public string SlackDownloadUrl =>
    _externalServicesConfigSection["Slack:DownloadUrl"] ?? throw new MemberAccessException(
      "Slack Download Url is missing in appsetting.json file");
  public string SlackInvitationUrl =>
    _externalServicesConfigSection["Slack:InvitationUrl"] ?? throw new MemberAccessException(
      "Slack Invitation Url is missing in appsetting.json file");
  public string UserManualUrl =>
    _externalServicesConfigSection["UserManualUrl"] ?? throw new MemberAccessException(
      "User Manual Url is missing in appsetting.json file");
